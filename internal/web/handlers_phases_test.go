...
// TestWaves_PageRenders: GET /p/{slug}/waves renders the page (no tickets so
// empty hint).
func TestWaves_PageRenders(t *testing.T) {
	srv, client, deps := freshServerWithDeps(t)
	repo := seedRepoOnDisk(t, t.TempDir(), "wv", "waves-proj")
	if _, err := deps.Service.RegisterProjectMount(context.Background(), repo); err != nil {
		t.Fatalf("RegisterProjectMount: %v", err)
	}
	resp, err := client.Get(srv.URL + "/p/waves-proj/waves")
	if err != nil {
		t.Fatalf("GET: %v", err)
	}
	body := mustReadAll(t, resp)
	...
	if !strings.Contains(body, "Waves") {
		t.Errorf("waves page missing title\n%s", body)
	}
}
